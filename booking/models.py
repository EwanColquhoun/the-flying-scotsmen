...
class Booking(models.Model):
    ...
    def get_html_url(self):
        ...
        if str(self.aircraft) == 'G-BTXG':
            return f'<a class="btn-events aircraft-purple mt-2 calendar-events" name={self.date.month}_{self.date.day} href={url}>{self.slot} | {self.username}</a>'
        else:
            return f'<a class="btn-events aircraft-green mt-2 calendar-events" name={self.date.month}_{self.date.day} href={url}>{self.slot} | {self.username}</a>'